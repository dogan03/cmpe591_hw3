...
if __name__ == "__main__":
    ...
    model_path = "saved_models_reinforce_2/checkpoint_8000.pt"
    episodes = 10
    ...
    if os.path.exists(model_path):
        ...
        print(
            f"Model loaded - trained until episode {checkpoint.get('episode', 'unknown')}"
        )
    else:
    ...
    os.makedirs("reinforce_gifs", exist_ok=True)
    for i, frames in enumerate(results["frames"]):
        if frames:
            gif_path = f"reinforce_gifs/episode_{i+1}.gif"
